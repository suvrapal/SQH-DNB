...
sim_data_Wei=function(n,g1,g2,delta,eta1,p_low,p_high,phi){
  ...
  for(i in 1:n){
    ...
    m=rnbinom(1,size=(1/phi),prob=(1/(1+(phi*eta[i]))))
    if(m==0){
      D[i]=0
    }else{
    ...
    #D[i]=rpois(1,lambda=(eta[i]*p[i]*exp(2*phi)))
...
log.lik = function(b21,b10,b11,g1,g2,phi,data_obs,data_cens,zt,zc,xt,xc){ 
  ...
  Spt = (1/(1+(phi*etat*pt*Ft)))^(1/phi)
  Spc = (1/(1+(phi*etac*pc*Fc)))^(1/phi)
  fpt = (etat*pt/(1+(phi*etat*pt*Ft)))*Spt*ft
...
l.aug.phi = function(par=c(phi1),b21,b10,b11,g1,g2,phi,data_obs,data_cens,zt,zc,xt,xc,eps){ 
  ...
  Fc = 1-exp(-((g2*data_cens)^(1/g1)))
  ft = (1/(data_obs*g1))*((g2*data_obs)^(1/g1))*(1-Ft)
  Spt = (1/(1+(par[1]*etat*pt*Ft)))^(1/par[1])
  Spc = (1/(1+(par[1]*etac*pc*Fc)))^(1/par[1])
  fpt = (etat*pt/(1+(par[1]*etat*pt*Ft)))*Spt*ft
...
SQH_DNB_Wei=function(data_obs,data_cens,zt,zc,xt,xc,tol,maxit,b21,b10,b11,g1,g2,phi,epsilon1,lambda1,zeta1,rho1){
  ...
  while(continue){
    ...
    if(0%in%p.new | p.new[4]<0 | p.new[5]<0 | p.new[5]<0 | p.new[6]<0){
      p.new = c(0,0,0,0,0,0)
      continue = FALSE
    }else{
...
log.lik.fun = function(pnew=c(b21,b10,b11,g1,g2,phi),data_obs,data_cens,zt,zc,xt,xc){ 
  ...
  Spt = (1/(1+(pnew[6]*etat*pt*Ft)))^(1/pnew[6])
  Spc = (1/(1+(pnew[6]*etac*pc*Fc)))^(1/pnew[6])
  fpt = (etat*pt/(1+(pnew[6]*etat*pt*Ft)))*Spt*ft
...
lambda = function(pars,d.k,g.k,data_obs,data_cens,zt,zc,xt,xc,del){
  ...
  while(cont){
    lam.k = 1/(2^(k-1))
    pp = pars+(d.k*lam.k)
    pp.new = c(pp[1],pp[2],pp[3],max(pp[4],0.01),max(pp[5],0.01),max(pp[6],0.01))
...
NCG_DNB_Wei=function(data_obs,data_cens,zt,zc,xt,xc,tol,maxit,b21,b10,b11,gam1,gam2,phi,del){
  ...
  while(continue){
    ...
    if(lam.vec[2]>20 | g.old[1]==0 | dum1.new[1]==0){
      p.new = c(0,0,0,0,0,0)
...
for(i in 1:n){
  ...
  nr_sqh = SQH_DNB_Wei(data_obs=data_obs,data_cens=data_cens,zt=zt,zc=zc,xt=xt,xc=xc,tol=tol,maxit=1000,b21=b21_init,b10=b10_init,b11=b11_init,
                       g1=g1_init,g2=g2_init,phi=phi_init,epsilon1=100,lambda1=100,zeta1=0.5,rho1=100)
  
  nr_ncg = NCG_DNB_Wei(data_obs,data_cens,zt,zc,xt,xc,tol,maxit,b21_init,b10_init,b11_init,g1_init,g2_init,phi_init,del)
